...
const game = {
    ...
    customers: [],
    stations: [
        { id: 1, name: 'Manicure', icon: '💅', slots: [null], maxSlots: 1 },
        { id: 2, name: 'Pedicure', icon: '🦶', slots: [null], maxSlots: 1 },
        { id: 3, name: 'Facial', icon: '💆‍♀️', slots: [null], maxSlots: 1 },
        { id: 4, name: 'Hair Styling', icon: '💇‍♀️', slots: [null], maxSlots: 1 }
    ],
    customerIdCounter: 0,
    upgrades: {
        massageChairs: { level: 0, maxLevel: 5 },
        snackBar: { level: 0, maxLevel: 5 },
        drinkStation: { level: 0, maxLevel: 5 },
        atm: { level: 0, maxLevel: 3 }
    },
    passiveIncome: 0,
...
// Service definitions
const services = [
    { name: 'Manicure', icon: '💅', duration: 5000, price: 50, station: 1 },
    { name: 'Pedicure', icon: '🦶', duration: 6000, price: 60, station: 2 },
    { name: 'Facial', icon: '💆‍♀️', duration: 7000, price: 80, station: 3 },
    { name: 'Hair Styling', icon: '💇‍♀️', duration: 8000, price: 100, station: 4 }
];
...
const upgradeDefinitions = {
    ...
    snackBar: {
        name: 'Snack Bar',
        icon: '🍪',
        description: 'Delicious snacks keep customers happy and generate income',
        baseCost: 300,
        costMultiplier: 1.6,
        incomePerLevel: 8,
        patienceBonus: 15,
    ...
    drinkStation: {
        name: 'Drink Station',
        icon: '🧃',
        description: 'Refreshing beverages for your customers',
        baseCost: 250,
        costMultiplier: 1.5,
        incomePerLevel: 6,
        patienceBonus: 12,
    ...
    atm: {
        name: 'ATM Machine',
        icon: '🏧',
        description: 'Customers can withdraw cash, increasing tips!',
        baseCost: 500,
        costMultiplier: 2,
        incomePerLevel: 15,
        tipBonus: 25,
...
const customerNames = [
    ...
    'Amelia', 'Harper', 'Evelyn', 'Abigail', 'Emily', 'Luna', 'Aria',
...
function calculatePatienceBonus() {
    let bonus = 0;
    if (game.upgrades.massageChairs.level > 0) {
        bonus += upgradeDefinitions.massageChairs.patienceBonus * game.upgrades.massageChairs.level;
    }
    if (game.upgrades.snackBar.level > 0) {
        bonus += upgradeDefinitions.snackBar.patienceBonus * game.upgrades.snackBar.level;
    }
    if (game.upgrades.drinkStation.level > 0) {
        bonus += upgradeDefinitions.drinkStation.patienceBonus * game.upgrades.drinkStation.level;
    }
    return bonus;
}
...
function calculateTipBonus() {
    let bonus = 0;
    if (game.upgrades.atm.level > 0) {
        bonus += upgradeDefinitions.atm.tipBonus * game.upgrades.atm.level;
    }
    return bonus;
}
